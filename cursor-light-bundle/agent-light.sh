#!/bin/bash
# Cursor / Claude Code Agent BLE 红绿灯 — 核心逻辑（ble_gate.py 原子去重）
#   HOOK_SOURCE=claude-code  → Claude Code stdin 格式
#   否则默认 Cursor 格式
...
case "$ACTION" in
  idle)
    if [[ "${HOOK_SOURCE:-}" != "claude-code" ]]; then
  ...
  stop)
    ...
    if [[ "${HOOK_SOURCE:-}" == "claude-code" ]]; then
      # Claude Code: 简化 stop 逻辑，不涉及 Plan/Build
      case "$status" in
        completed)
  ...
  denied)
    if [[ "${HOOK_SOURCE:-}" == "claude-code" ]]; then
      # Claude Code postToolUseFailure: 统一当 error 处理
